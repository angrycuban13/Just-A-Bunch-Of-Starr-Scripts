...
foreach ($app in $apps) {
    ...
    if ($app -eq "radarr") {
        ...
        if ($filteredMovies.Count -eq 0 -and $unattended -eq $false) {
            throw "No movies left to search!"
        }

        elseif ($filteredMovies.Count -eq 0 -and $unattended -ne $false) {

            $updatedFilteredMovies = $allMovies | Where-Object {$_.tags -contains $tagId -and $_.monitored -eq $config.Radarr.radarrMonitored -and $_.status -eq $config.Radarr.radarrMovieStatus}

            Write-Verbose "Removing tag `"$($config.Radarr.radarrTagName)`" from $($updatedFilteredMovies.count) movies."

            Remove-Tag -App $app -Url $($config.($app)."$($app)Url") -TagName $($config.$($app)."$($app)TagName") -Movies $updatedFilteredMovies -TagId $tagID

            if($config.Radarr.radarrCount -eq "max") {

                $config.Radarr.radarrCount = $updatedFilteredMovies.count

                Write-Verbose "Radarr count set to max. `nMovie count has been modified to $($updatedFilteredMovies.count) movies."
            }

            $randomMovies = Get-Random -InputObject $updatedFilteredMovies -Count $config.Radarr.radarrCount
    ...
    if ($app -eq "readarr") {
        throw "What does this even do?"
    }

    if  ($app -eq "sonarr") {
        throw "Write Markus answer here"
    }

    if ($app -eq "whisparr") {
        throw "Does Q even work on this?"
    }
}
